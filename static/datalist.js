function datalist_init (input) {
  ...
  if (url.length || urlfn.length) {
    ...
    datalist_match = function (s, fn, ev) {
      ...
      timer = setTimeout(function () {
        ...
        x.onreadystatechange = function () {
          if (x.readyState != 4 || [ 0, 200 ].indexOf(x.status) == -1) { return }

          prevmatches = []
          var o = JSON.parse(x.responseText)
          for (var i = 0; i < o.length; i++) { prevmatches.push(createitem(o[i])) }
    ...
    // use inline <datalist>.
    if (attrlist === null || ellist === undefined) { return }
        ...
        for (var k = 0; k < tok.length && fc < tok.length; k++) {
          ...
          for (var j = 0; j < data[i].search.length && fc < tok.length && !f; j++) {
            for (var l = 0; l < data[i].search.length && !f; l++) {
              if (data[i].search[l].indexOf(tok[k]) != -1) { f = true }
            }
          }
          if (f) { fc++ }
        }
        // all tokens (separated by space) must match.
        if (fc == tok.length) { matches.push(data[i]) }
      }
      return matches
    }
    ...
    datalist_match = function (s, fn) {
      ...
      if (prevvalue === null || (prevvalue.split(' ').length != s.split(' ').length) ||
			    s.indexOf(prevvalue) == -1) { prevmatches = datalist_filter(items, s) } else { prevmatches = datalist_filter(prevmatches, s) }
      prevvalue = s
      fn(prevmatches)
    }
  }
  ...
  var datalist_render = function (m) {
    ...
    for (var i = 0; i < m.length; i++) { dd.appendChild(m[i].el) }
  ...
  input.addEventListener('keydown', function (e) {
    ...
    switch (e.which) {
      case 13: // return
        if (cursel) { input.value = getvalue(cursel) }
        if (!datalist_visible) { return }
        datalist_show(false)
        e.stopPropagation()
      ...
      case 40: // arrow down
        ...
        if (dc.length) {
          if (e.which == 38) { // up
            if (!sel || !(sel = sel.previousSibling)) { sel = dc[dc.length - 1] }
          } else if (e.which == 40) { // down
            if (!sel || !(sel = sel.nextSibling)) { sel = dc[0] }
          } else if (!sel) {
            sel = dc[0]
          }
        }
        ...
        if (sel) {
          ...
          if (sel.offsetTop < dd.scrollTop) { dd.scrollTop = sel.offsetTop } else if (sel.offsetTop + sel.offsetHeight > dd.scrollTop + dd.offsetHeight) { dd.scrollTop = sel.offsetTop }
        }
    }
  }, false)
    ...
    datalist_match(input.value, function (m) {
      ...
      if (cursel) {
        ...
        for (var i = 0; i < m.length && !(hassel = (m[i].el === cursel)); i++)
          ;
        if (!hassel) { datalist_setsel(null) }
      }
      // only one match? select it.
      if (m.length == 1) { datalist_setsel(m[0].el) }
      datalist_render(m)
...
if (els !== null) {
  for (var i = 0; i < els.length; i++) { datalist_init(els[i]) }
}
